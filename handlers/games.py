...
• inline-список игр, детализация, возврат «Назад»
• per-user async-lock, vacuum старых сообщений

Логика полностью соответствует MasterBot 12.92, только
исправлены ошибки импорта и добавлен helper _delete_trigger.
"""
...
from services.gsheets import get_user_status_from_svetofor  # noqa: F401 (может пригодиться)
...
async def user_lock(uid: int):
    """Глобальная блокировка для каждого пользователя для предотвращения гонок."""
...
async def _delete_trigger(msg: types.Message) -> None:
    """
    Пробует удалить сообщение; если не получается, сохраняет его в state.messages_to_delete,
    чтобы удалить позже.
    """
    try:
        await msg.delete()
...
def _truncate(text: str, limit: int = 100) -> str:
    """Обрезает текст до limit символов с добавлением '…'."""
    return text if len(text) <= limit else text[: limit - 1] + "…"


async def _refresh_menu(user_id: int) -> None:
    """
    Перерисовывает главное Reply-меню, удаляя предыдущее.
    Используется после открытия inline-экранов.
    """
    from handlers.polls import get_main_menu  # локальный импорт, чтобы избежать цикла
...
async def show_games(
    ...
    """
    Строит inline-список подходящих игр (максимум 20 штук) и сохраняет
    его в state.games_by_user для последующей детализации.
    """
    bot = Bot.get_current()
    ...
    async with user_lock(user_id):
        ...
        if not deals:
            logger.error("[games] Failed to retrieve deals from AmoCRM for user %d", user_id)
            await message.answer(f"{title}\n⚠️ Не удалось загрузить данные игр. Проверьте подключение к AmoCRM.")
            return
        ...
        sent = await bot.send_message(user_id, f"{title}\nВыберите игру:", reply_markup=kb.as_markup())
...
async def new_games_handler(message: types.Message) -> None:
    ...
    if ui and ui["role"] in settings.ACCESS["games"]:
        await show_games(message, uid, settings.NEW_GAMES_STATUS_IDS, "📅 Новые игры:", True)
    else:
...
async def assigned_games_handler(message: types.Message) -> None:
    ...
    ui = await get_user_info(uid)
    if ui and ui["role"] in settings.ACCESS["games"]:
        await show_games(message, uid, [settings.SUCCESSFUL_STATUS_ID], "✅ Распределённые игры:")
    else:
        await message.answer("⛔ Нет доступа.")
    await _delete_trigger(message)
...
async def game_details_handler(callback: types.CallbackQuery) -> None:
    ...
    try:
        ...
        deal = next((d for d in state.games_by_user.get(user_id, []) if d["id"] == deal_id), None)
        if not deal:
        ...
        text = "\n".join([
            ...
            f"🧑‍🤝‍🧑 *Ведущие*: {', '.join(l['name'] for l in deal.get('team_leads',[])) or '—'}",
        ...
        sent = await bot.send_message(user_id, text, parse_mode="Markdown", reply_markup=kb.as_markup())
        state.last_user_messages[user_id] = [sent]
        await callback.answer()
    except Exception as e:
        logger.error("[games] game_details_handler: %s", e, exc_info=True)
...
async def back_to_games_list_handler(callback: types.CallbackQuery) -> None:
    ...
    try:
        ...
        sent = await callback.message.answer("📋 Список игр:\nВыберите игру:", reply_markup=kb.as_markup())
        state.last_user_messages[user_id] = [sent]
        await callback.answer()
    except Exception as e:
        logger.error("[games] back_to_games_list: %s", e, exc_info=True)
